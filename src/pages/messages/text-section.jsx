...
export function ChatList() {
    ...
    return (
        <div className="flex flex-col h-full rounded-sm overflow-hidden border border-primary">
            <div className="flex justify-between items-center gap-4 mb-2">
                <div className="text-xs font-bold p-2 px-5 border rounded-md flex-1 text-center relative">
                    <span className="absolute py-1 p-2 bg-primary text-white text-xs rounded-md right-1 top-1">
                        1
                    </span>
                    All
                </div>
                <div className="text-xs font-bold p-2 px-5 border rounded-md flex-1 text-center relative">
                    <span className="absolute py-1 p-2 bg-[#0FFF9A] text-xs rounded-md right-1 top-1">
                        1
                    </span>
                    Read
                </div>
                <div className="text-xs font-bold p-2 px-5 border rounded-md flex-1 text-center relative">
                    <span className="absolute py-1 p-2 bg-[#FFDE16] text-xs rounded-md right-1 top-1">
                        1
                    </span>
                    Unread
                </div>
            </div>
            <div className="p-2.5 px-4 bg-primary text-white text-sm font-semibold">
                New Messages
            </div>
            <div className="pt-2 px-2 relative">
                <IoIosSearch className="absolute top-3.5 left-3.5 text-3xl" />
                <input
                    className="w-full border-2 border-gray-400 p-2 rounded-full px-4 text-sm pl-10"
